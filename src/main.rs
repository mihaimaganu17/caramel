...
fn flatten_pe<P: AsRef<Path>>(filename: P) -> Option<(u32, u32, Vec<u8>)> {
    ...
    let image_size: usize = (image_end - image_start + 1).try_into().ok()?;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    std::fs::write(Path::new("build").join("caramel.flat"), image)?;
    ...
    // Make sure our bootloader is not too big for PXE
    if bl_size > MAX_BOOTLOADER_SIZE {
        return Err("Bootlaoder size is too large".into());
    }
