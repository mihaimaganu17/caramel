...
/// An inclusive range. We do no use `RangeInclusive` as it does not impl Copy
#[derive(Clone, Copy)]
#[repr(C)]
pub struct Range {
    pub start: u64,
...
impl RangeSet {
    ...
    /// the set becoming empty, the range will be removed entirely from the set
    pub fn remove(&mut self, range: Range) {
        assert!(range.start <= range.end, "Invalid range shape");

        'try_substractions: loop {
            for i in 0..self.in_use as usize {
                let ent = self.ranges[i];
                ...
                if range.start <= ent.start {
                    ...
                    // need to split it into 2 ranges.
                    self.ranges[i].start = range.end.saturating_add(1);
    ...
    /// Substracts a `RangeSet` from `self`
    pub fn substract(&mut self, rs: &RangeSet) {
    ...
    pub fn allocate(&mut self, size: u64, align: u64) -> Option<usize> {
        ...
        // Validate alignment is nonzero and power of 2
...
/// Returns true if the entirity of [x1, x2] is contained inside [y1, y2],
/// else returns false
